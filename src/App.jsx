...
const App = () => {
  ...
  return (
    <div
      className={`min-h-screen min-w-full bg-neutral-800 text-white flex flex-col items-center ${
        !user ? "justify-center" : ""
      }`}
    >
      {/* presistant navbar */}
      <nav
        className={`top-0 w-full py-2 px-4 bg-neutral-900 flex items-center ${
          !user
            ? "justify-between absolute top-0"
            : "justify-between absolute top-0"
        }`}
      >
        {!user ? (
          <>
            ...
            <h1>
              Welcome <span className="text-cyan-500"></span>
            </h1>
          </>
        ) : (
          <>
            ...
            <div className="flex items-center gap-1">
              <h1>
                Hi <span className="text-cyan-400">{user}</span>
              </h1>
              <img src={cyanUser} alt="" className="w-8" />
              <button
                onClick={leave}
                className="text-sm rounded-lg bg-cyan-500 px-2 py-1"
              >
                Leave
              </button>
            </div>
      ...
      {!user ? (
        <>
          ...
          <div className="rounded-2xl bg-neutral-600 w-80 flex flex-col items-center justify-evenly p-6 gap-6">
            <div className="">
              <h1 className="text-xl text-center inline">Come!</h1>
            </div>
            <div className="w-full rounded-xl flex flex-col gap-1 items-center">
              <input
                ...
                id="username"
                value={authUsername}
                onChange={(e) => setAuthUsername(String(e.target.value))}
                placeholder="Username"
                className="w-11/12 py-1 px-2 leading-8 rounded-lg text-black  focus:outline-none"
              />
            ...
            <div className="w-1/4">
              <button
                onClick={getIn}
                className="w-full bg-cyan-500 rounded-lg px-3 py-1 hover:bg-cyan-600 hover:scale-105 duration-150"
              >
                Enter!
              </button>
            </div>
          ...
          <div className="mt-20 min-w-full flex flex-col items-center p-5 gap-5">
            ...
            <div className="flex justify-center gap-2 rounded-lg w-11/12 sm:w-7/12 md:w-1/2 lg:w-1/3 xl:w-1/3">
              <img
                src={cyanUser}
                alt=""
                className="w-10 h-10 p-1 rounded-full bg-neutral-600"
              />
              ...
              <div className="flex flex-col bg-neutral-700 rounded-2xl p-2 items-end w-full">
                <textarea
                  ...
                  className="w-full rounded-2xl px-5 py-2 focus:outline-none bg-neutral-700"
                  placeholder="Write your mind here..."
                />

                <button
                  onClick={postComment}
                  className="flex gap-2 items-center text-base rounded-lg bg-cyan-500 px-2 py-1"
                >
                  Post
                  <IoSend />
                </button>
